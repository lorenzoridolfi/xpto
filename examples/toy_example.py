...
def validate_manifest_for_toy_example(
    ...
    """
    Load and validate the shared manifest file for toy_example.
    Args:
        manifest_path (str): Path to the manifest file
        schema_path (str): Path to the manifest schema
    Returns:
        dict: Loaded manifest
    Raises:
        FileNotFoundError: If the manifest file does not exist
        json.JSONDecodeError: If the manifest is not valid JSON
        jsonschema.ValidationError: If the manifest does not conform to the schema
    """
    if not os.path.exists(manifest_path):
        raise FileNotFoundError(
            f"Manifest file not found: {manifest_path}. Please run update_manifest.py first."
        )
    ...
    try:
        ...
        raise jsonschema.ValidationError(f"Manifest validation failed: {e.message}")
    return manifest
