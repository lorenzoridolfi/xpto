...
def remove_keys_from_json(data, keys_to_remove_lower):
    if isinstance(data, dict):
        return {
            ...
            for k, v in data.items() if k.lower() not in keys_to_remove_lower
        }
...
def main():
    parser = argparse.ArgumentParser(description="Remove specified keys from a JSON file (case-insensitive matching).")
    parser.add_argument("input_json", help="Path to the input JSON file")
    parser.add_argument("output_json", help="Path to the output JSON file")
    parser.add_argument("keys_txt", help="Path to the text file containing keys to remove")
