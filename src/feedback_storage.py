    ...
    async def get(self, entry_id: UUID) -> Optional[FeedbackEntry]:
        ...
        try:
            ...
            if entry:
                ...
                self.log_warning("Entry not found", entry_id=str(entry_id))
            return entry

        except Exception as e:
            self.log_error(
                "Failed to retrieve entry", entry_id=str(entry_id), error=str(e)
            )
    ...
    async def purge_by_count(self, keep_last_n: int) -> int:
        ...
        try:
            ...
            sorted_entries = sorted(
                self._storage.items(), key=lambda x: x[1].timestamp, reverse=True
            )
    ...
    def get_storage_stats(self) -> StorageStats:
        ...
        try:
            stats: StorageStats = {
                ...
                "entries_with_feedback": sum(
                    1 for e in self._storage.values() if e.feedback
                ),
                "oldest_entry": min(
                    (e.timestamp for e in self._storage.values()), default=None
                ),
                "newest_entry": max(
                    (e.timestamp for e in self._storage.values()), default=None
                ),
